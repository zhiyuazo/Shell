function refreshkeycloakuser
{
        ...
        if [[ $DBtype == 0 ]]; then
            ...
            if [[ $H2KCuser_flag == 0  ]]; then
                ...
                $CKEYHOME/bin/add-user-keycloak.sh  -r master -u admin -p admin --sc /opt/keycloak/domain/servers/server-1/configuration >/dev/null 2>&1
                chown keycloak:keycloak $CKEYHOME/domain/servers/server-1/configuration/keycloak-add-user.json 
